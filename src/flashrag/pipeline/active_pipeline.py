...
from verl.utils.reward_score.lets import remove_boxed, last_boxed_only_string, extract_answer

class LeTSPipeline(BasicPipeline):
    def __init__(self, config, retriever=None, generator=None, apply_chat=False):
        ...
        if apply_chat:
            self.prompt_template = prompt_template_dict['lets_template_sys']
        else:
            self.prompt_template = prompt_template_dict['lets_template']
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        with ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc="Inference: ", ncols=50): # 始终50列
                future.result()
