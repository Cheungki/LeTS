...
def validate_format(text: str) -> tuple[bool, str, List[str]]:
    search_path = []
    ...
    while True:
        ...
        search_path.append(text[search_pos+8:search_end_pos].strip())
    ...
    return True, "format is correct", search_path
...
def compute_score(tokenizer, solution_str, ground_truth) -> tuple[float, str, List[str]]:
    ...
    valid_template, reason, search_path = validate_format(response)
    if not valid_template:
        return 0, f'bad format: {reason}', search_path

    if response.endswith(tokenizer.eos_token):
        response = response[:-len(tokenizer.eos_token)]
    else:
        return 0, 'over length', search_path
    ...
    if answer_part is not None:
        try:
            ...
            return 0, f'find box error: {e}', search_path
    else:
        return 0, 'cannot extract answer', search_path

    f1_score = get_f1_score(answer, ground_truth)
    if f1_score > 0:
        return 0.1 + 0.9 * f1_score, f'correct answer, get f1 score: {f1_score}', search_path
    else:
        return 0.1, f'wrong answer but good format: {answer}', search_path