...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_lets = config.get('use_lets', True)
        self.prompt_template_name = config.get('prompt_template_name', 'lets_template_sys')
    ...
    def _pack_lets_input(self, prompt_template, user_input):
        if self.prompt_template_name == 'lets_template_sys':
            return self.tokenizer.apply_chat_template([
                    {'role': 'system', 'content': prompt_template}, 
                    {'role': 'user', 'content': user_input}
                ], add_generation_prompt=True, tokenize=False)
        else:
            return prompt_template.format(prompt=user_input)
        ...
        if self.filter_overlong_prompts:
            ...
            if self.use_lets:
                self.dataframe = self.dataframe.filter(
                    lambda doc: len(tokenizer.encode(self._pack_lets_input(self.prompt_template, doc[prompt_key]))
    ...
    def __getitem__(self, item):
        ...
        if self.use_lets:
            prompt_with_chat_template = self._pack_lets_input(self.prompt_template, chat)
        else:
