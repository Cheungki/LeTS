...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if 'use_hybrid' not in data.meta_info.keys() or not data.meta_info['use_hybrid']:
            advantages, returns = core_algos.compute_grpo_outcome_advantage(
                token_level_rewards=data.batch['token_level_rewards'],
                response_mask=data.batch['response_mask'],
                index=data.non_tensor_batch['uid'])
        else:
            advantages, returns = core_algos.compute_hybrid_grpo_advantage(
                token_level_rewards=data.batch['token_level_rewards'],
                process_level_rewards=data.non_tensor_batch['stepwise_rewards'],
...
class RayPPOTrainer(object):
    ...
    def _save_best_checkpoint(self):
        ...
        print(f'best_checkpoint_folder: {best_checkpoint_folder}')
        
        os.makedirs(best_checkpoint_folder, exist_ok=True)
        self.actor_rollout_wg.save_checkpoint(best_checkpoint_folder, is_save_best=True)

        # latest checkpointed iteration tracker (for atomic usage)
        local_best_checkpointed_iteration = os.path.join(self.config.trainer.default_local_dir,
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        try:
                            if not self.config.reward_model.enable_stepwise_reward:
                                reward_result = self.reward_fn(
                                                    batch,
                                                    return_dict=True,
                                                    curr_save_path=os.path.join(self.config.trainer.rollout_save_path, f'train_{self.global_steps}.jsonl')
                                                )
                                batch.meta_info['use_hybrid'] = False
                            else:
                                reward_result = self.reward_fn(
                                                    batch,
                                                    return_dict=True,
                                ...
                                batch.non_tensor_batch['stepwise_rewards'] = np.array(reward_result['stepwise_rewards'], dtype=object)
                                batch.meta_info['use_hybrid'] = True
                            reward_tensor = reward_result['reward_tensor']
                            reward_extra_infos_dict = {} if 'reward_extra_info' not in reward_result.keys() else reward_result['reward_extra_info']
                    ...
                    if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and \
                        ...
                        #     if k.endswith('mean@1'):
                        #         curr_score = v
                        #         break
                        # if best_score <= curr_score:
                        #     print(f'At step {self.global_steps}: Best score updated from {best_score} to {curr_score}')
