...
def compute_hybrid_grpo_advantage(token_level_rewards: torch.Tensor,
                                  process_level_rewards: List[list],
    ...
    """Implementation Details
        - hybrid-then-standardize: advantage_{i,j} = standardize(r_o + r_p)
        - standardize-then-hybrid: advantage_{i,j} = standardize(r_o) + standardize(r_p)
        - common issues
            - hybrid ratio
            - rollouts without stepwise rewards
                - wrong rollouts: 
                - neglected rollouts
    """
    ...
    with torch.no_grad():
        ...
        for i in range(loss_mask.shape[0]):
            stepwise_rewards = process_level_rewards[i]
            if len(stepwise_rewards) <= 1:
                continue

            non_zero_indices = torch.nonzero(loss_mask[i]).squeeze()
            intervals = []
            start_idx = non_zero_indices[0].item()
            for j in range(1, len(non_zero_indices)):
                if non_zero_indices[j] != non_zero_indices[j-1] + 1:
                    intervals.append((start_idx, non_zero_indices[j-1].item()))
                    start_idx = non_zero_indices[j].item()
            intervals.append((start_idx, non_zero_indices[-1].item()))

            stepwise_rewards = torch.tensor(stepwise_rewards)
            stepwise_ratios = (stepwise_rewards - torch.mean(stepwise_rewards)) / (torch.std(torch.tensor(stepwise_rewards)) + epsilon)

            for stepwise_ratio, interval in zip(stepwise_ratios, intervals[:-1]):
                if scores[i][interval[0]] >= 0:
                    scores[i][interval[0]:interval[1]+1] *= (stepwise_ratio * gamma + 1)
                else:
                    scores[i][interval[0]:interval[1]+1] *= (-stepwise_ratio * gamma + 1)
